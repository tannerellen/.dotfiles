...
# ifconfig.me gets external IP then ipapi gets the location from that
...
# Remove excess whitespace and other formatting
output=$(echo "$weather" | \
    sed -e 's/  \+/ /g' \
        -e 's/+//g' \
        -e 's/°C//g' \
        -e 's/°F/°/g')  # Optional: If you want to keep the degree symbol
