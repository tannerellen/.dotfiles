...
if [[ $mode == "region" ]]; then
	grim -g "$(slurp)" - | satty --filename - --fullscreen --early-exit --output-filename "$filePath"
else
	grim -g "$(slurp -o -r)" - | satty --filename - --fullscreen --early-exit --output-filename "$filePath"
fi
